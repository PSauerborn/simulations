...
def load_output_data(simulation: Simulation, output_dir: Path) -> pl.DataFrame:
    """Load the output data from the output directory.

    Reads the CSV output file generated by the simulation and renames
    columns based on the simulation output type. The output file path is
    constructed by joining output_dir with the simulation's output_path.

    Args:
        simulation: The simulation whose output data should be loaded.
        output_dir: Directory containing the simulation output files.

    Returns:
        A DataFrame containing the simulation output with named columns.
    """

    path = output_dir / simulation.output_path
...
def run_simulation(simulation: Simulation, binary_path: str, output_dir: Path):
    ...
    Executes the simulation using fpm, passing the output directory as a
    command-line argument to the Fortran executable. Stdout is captured
    ...
    Args:
        ...
        output_dir: Directory where output files (data, logs) should be written.
            This path is passed to the Fortran executable as a CLI argument.
...
def main(args):
    ...
    df = load_output_data(simulation, run_output_dir)
    match simulation.output_type:
        case SimulationOutputTypeEnum.TRAJECTORY:
            plot_trajectory(df, run_output_dir)
            log.info("Plots saved.", output_dir=str(run_output_dir))
